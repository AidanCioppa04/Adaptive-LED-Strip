/*
This implimentation is the base proposed idea as outlined in the docs
This system uses a motion sensor to toggle the strip on/off
                 a light sensor to set the brightness of the strip
                 an addressable LED strip to provide lighting

The system initializes to off, requiring a motion detection to turn on
Upon initilization the light sensor will begin scaling the brightness of the
strip to adapt to ambient lighting, wether the strip is on or off
The LED strip will display all red for base implimentation, color/design changes 
require hard-coding into the "stripRedSolid()" function.
*/
